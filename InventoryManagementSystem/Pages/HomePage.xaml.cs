		...
		private void productDataGrid_CellEditEnding(object sender, DataGridCellEditEndingEventArgs e)
		{
			...
			switch (propertyName)
			{
				...
				case "USDPrice":
					...
					if (double.TryParse(StringHelper.TrimAllWhiteSpaces(editedValue), CultureInfo.InvariantCulture, out double usdPrice))
					{
				...
				case "USDPriceForCustomer":
					...
					if (double.TryParse(StringHelper.TrimAllWhiteSpaces(editedValue), CultureInfo.InvariantCulture, out double usdPriceForCustomer))
					{
